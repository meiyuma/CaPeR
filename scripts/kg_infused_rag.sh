...
for TASK in "${TASK_ORDER[@]}"; do
    ...
    for ((i=0; i<${#CORPUS_CONFIGS[@]}; i+=3)); do
        ...
        for LLM_MODEL in "${LLM_MODELS[@]}"; do
            ...
            # Log
            LOG_DIR="./logs/kg_infused_rag/${TASK}"
            mkdir -p "$LOG_DIR"
            LOG_FILE="$LOG_DIR/kg_qe_${KG_BASED_QE}_kg_aug_ag_${KG_AUG_AG}_rounds_${N_MAX_EXPLORE_ROUNDS}_${LLM_MODEL_NAME}_${CORPUS_SOURCE_NAME}_$(date +'%Y-%m-%d_%H-%M-%S').txt"
            ...
            echo "  - RETRIEVAL_MODEL = $RETRIEVAL_MODEL"
